...
public class NebulaCraterFeature extends Feature<NoneFeatureConfiguration> {
	...
	public boolean place(FeaturePlaceContext<NoneFeatureConfiguration> context) {
		...
		BlockPos surface = world.getHeightmapPos(Heightmap.Types.WORLD_SURFACE_WG, new BlockPos(centerX, 0, centerZ)).below();
		if (surface.getY() <= world.getMinY() + 10) {
			return false;
		}
		...
		if (!world.isEmptyBlock(surface.above())) {
			return false;
		}
	...
	private static boolean carveCrater(WorldGenLevel world, BlockPos centerSurface, int radius, RandomSource random) {
		int centerX = centerSurface.getX();
		int centerY = centerSurface.getY();
		int centerZ = centerSurface.getZ();

		int bottomY = world.getMinY() + 1;
		...
		for (int dx = -radius - 1; dx <= radius + 1; dx++) {
			for (int dz = -radius - 1; dz <= radius + 1; dz++) {
				...
				BlockPos sampleColumn = world.getHeightmapPos(Heightmap.Types.WORLD_SURFACE_WG, new BlockPos(columnX, 0, columnZ)).below();
				...
				if (!isEndTerrain(topState) && coreArea) {
					return false; // interior lacks end terrain to carve
				}
				...
				for (int y = sampleColumn.getY(); y > floorY; y--) {
					...
					if (!state.isAir()) {
						world.setBlock(mutable, Blocks.AIR.defaultBlockState(), Block.UPDATE_CLIENTS);
						modifiedAny = true;
					}
				}
				...
				if (isCarvable(floorState) || floorState.isAir()) {
					world.setBlock(mutable, ModBlocks.ASH_STONE.defaultBlockState(), Block.UPDATE_CLIENTS);
					modifiedAny = true;
				}
		...
		// paint crater walls with ash stone for a cohesive look
		BlockPos.MutableBlockPos wallCursor = new BlockPos.MutableBlockPos();
		...
		for (int dx = -radius - 1; dx <= radius + 1; dx++) {
			for (int dz = -radius - 1; dz <= radius + 1; dz++) {
				...
				for (int y = wallMaxY; y >= wallMinY; y--) {
					...
					world.setBlock(wallCursor, ModBlocks.ASH_STONE.defaultBlockState(), Block.UPDATE_CLIENTS);
				}
			}
		}

		BlockPos ventPos = new BlockPos(centerX, centerFloorY + 1, centerZ);
		BlockState belowVent = world.getBlockState(ventPos.below());
		if (!belowVent.is(ModBlocks.ASH_STONE)) {
			world.setBlock(ventPos.below(), ModBlocks.ASH_STONE.defaultBlockState(), Block.UPDATE_CLIENTS);
		}
		BlockState ventState = ModBlocks.NEBULA_VENT_BLOCK.defaultBlockState()
			.setValue(NebulaVentBlock.FACING, Direction.Plane.HORIZONTAL.getRandomDirection(random));
		world.setBlock(ventPos, ventState, Block.UPDATE_CLIENTS);

		// thicker ash stone band on the immediate rim for readability
		BlockPos.MutableBlockPos rimCursor = new BlockPos.MutableBlockPos();
		for (int dx = -radius - 1; dx <= radius + 1; dx++) {
			for (int dz = -radius - 1; dz <= radius + 1; dz++) {
				double dist = Math.sqrt((double) (dx * dx + dz * dz));
				...
				int rimX = centerX + dx;
				int rimZ = centerZ + dz;
				int rimY = world.getHeightmapPos(Heightmap.Types.WORLD_SURFACE_WG, new BlockPos(rimX, 0, rimZ)).getY();
				for (int y = rimY; y >= rimY - 2 && y > bottomY; y--) {
					...
					world.setBlock(rimCursor, ModBlocks.ASH_STONE.defaultBlockState(), Block.UPDATE_CLIENTS);
				}
			}
		}
	...
	private static boolean hasConsistentSurface(WorldGenLevel world, BlockPos centerSurface, int radius) {
		int centerY = centerSurface.getY();
		int minY = centerY;
		int maxY = centerY;
		int solidSamples = 0;
		...
		for (int angle = 0; angle < 360; angle += 45) {
			...
			BlockPos sampleSurface = world.getHeightmapPos(Heightmap.Types.WORLD_SURFACE_WG, new BlockPos(sampleX, 0, sampleZ)).below();
			if (sampleSurface.getY() <= world.getMinY() + 6) {
				return false;
			}
			...
			// avoid carving into end city/purpur structures
	...
	private static boolean hasOverheadObstruction(WorldGenLevel world, BlockPos centerSurface, int radius) {
		int centerY = centerSurface.getY();
		int maxDepth = Math.max(4, (int) Math.round(radius * 0.65));
		...
		for (int dx = -radius - 1; dx <= radius + 1; dx++) {
			for (int dz = -radius - 1; dz <= radius + 1; dz++) {
				...
				int columnTopY = world.getHeightmapPos(Heightmap.Types.WORLD_SURFACE_WG, surfacePos).getY();
	...
	private static boolean adjacentToAir(WorldGenLevel world, BlockPos.MutableBlockPos pos) {
	...
	private static void removeUnsupportedShadowClaws(WorldGenLevel world, BlockPos centerSurface, int radius) {
		BlockPos.MutableBlockPos mutable = new BlockPos.MutableBlockPos();
		int startY = centerSurface.getY() + 6;
		int minY = Math.max(world.getMinY() + 1, centerSurface.getY() - radius - 6);
		for (int dx = -radius; dx <= radius; dx++) {
			for (int dz = -radius; dz <= radius; dz++) {
				...
				for (int y = startY; y >= minY; y--) {
					mutable.set(x, y, z);
					BlockState state = world.getBlockState(mutable);
					if (state.is(ModBlocks.SHADOW_CLAW) && world.getBlockState(mutable.below()).isAir()) {
						world.setBlock(mutable, Blocks.AIR.defaultBlockState(), Block.UPDATE_CLIENTS);
					}
				}
			}
	...
	private static void removeFloatingMoldSpores(WorldGenLevel world, BlockPos centerSurface, int radius) {
		BlockPos.MutableBlockPos mutable = new BlockPos.MutableBlockPos();
		int startY = centerSurface.getY() + 6;
		int minY = Math.max(world.getMinY() + 1, centerSurface.getY() - radius - 6);
		for (int dx = -radius; dx <= radius; dx++) {
			for (int dz = -radius; dz <= radius; dz++) {
				...
				for (int y = startY; y >= minY; y--) {
					...
					if (state.is(ModBlocks.MOLD_SPORE) || state.is(ModBlocks.MOLD_SPORE_TUFT)) {
						BlockPos below = mutable.below();
						BlockState belowState = world.getBlockState(below);
						if (belowState.isAir() || !belowState.isSolid()) {
							world.setBlock(mutable, Blocks.AIR.defaultBlockState(), Block.UPDATE_CLIENTS);
						}
					} else if (state.is(ModBlocks.MOLD_SPORE_SPROUT)) {
						BlockPos below = mutable.below();
						BlockState belowState = world.getBlockState(below);

						// two-tall plant: mutable could be either half, check accordingly
						if (belowState.is(ModBlocks.MOLD_SPORE_SPROUT)) {
							BlockPos groundPos = below.below();
							BlockState groundState = world.getBlockState(groundPos);
							if (groundState.isAir() || !groundState.isSolid()) {
								world.setBlock(mutable, Blocks.AIR.defaultBlockState(), Block.UPDATE_CLIENTS);
								world.setBlock(below, Blocks.AIR.defaultBlockState(), Block.UPDATE_CLIENTS);
							}
						} else {
							if (belowState.isAir() || !belowState.isSolid()) {
								world.setBlock(mutable, Blocks.AIR.defaultBlockState(), Block.UPDATE_CLIENTS);
								...
								if (aboveState.is(ModBlocks.MOLD_SPORE_SPROUT)) {
									world.setBlock(above, Blocks.AIR.defaultBlockState(), Block.UPDATE_CLIENTS);
								}
							}
						}
					}
